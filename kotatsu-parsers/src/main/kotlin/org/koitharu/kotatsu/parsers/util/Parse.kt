...
public fun Response.parseHtml(): Document = use { response ->
    // NOTE (tachiyomi-legacy): parseamos desde String, NO desde InputStream.
    // Jsoup.parse(InputStream) usa SimpleStreamReader -> CharsetDecoder ICU, y el
    // decoder de Android 6.0 lanza IllegalArgumentException "Bad position" al
    // decodificar por stream (el mismo bug que rompe Kotatsu-Redo en API 23).
    // body.string() ya aplica el charset del Content-Type (fallback UTF-8).
