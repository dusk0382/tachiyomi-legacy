...
internal abstract class MangAdventureParser(
	...
	override suspend fun getDetails(manga: Manga): Manga {
		...
		val author = buildString {
			...
			// Android: org.json.JSONArray no es Iterable -> bucles por indice
			fun org.json.JSONArray.appendJoined(b: StringBuilder) {
				for (i in 0 until length()) {
					if (i > 0) b.append(", ")
					b.append(optString(i))
				}
			}
			if (authors.length() > 0 && artists.length() > 0) {
				authors.appendJoined(this)
				append(", ")
				artists.appendJoined(this)
		...
		return manga.copy(
			...
			tags = buildSet {
				val categories = details.getJSONArray("categories")
				for (i in 0 until categories.length()) {
					val name = categories.getString(i)
			...
			chapters = chapters?.optJSONArray("results")?.asTypedList<JSONObject>()?.mapChapters { _, it ->
				MangaChapter(
					...
					scanlator = it.getJSONArray("groups").toString().trim('[', ']').replace('\"', ' ').trim(),
