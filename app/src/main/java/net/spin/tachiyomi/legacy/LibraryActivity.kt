...
class LibraryActivity : AppCompatActivity() {
    ...
    private fun refreshLibrary() {
        if (currentTab == TAB_FAVORITES || currentTab == TAB_HISTORY) {
            loadTab()
            val msg = if (currentTab == TAB_FAVORITES) "Favoritos actualizados" else "Historial actualizado"
    ...
    /** Abre la ficha del manga (descripción + capítulos), p. ej. desde el historial. */
    ...
    private fun onOnlineLongClick(item: LibraryItem.Online) {
        when {
            ...
            currentTab == TAB_HISTORY -> {
                ...
                // Favoritos: mover a carpeta privada solo si SecureAddActivate está activo.
                val options = mutableListOf<String>()
                if (isMoveToPrivateEnabled) options.add("🔒 Mover a carpeta privada")
                options.add("Quitar de favoritos")
                AlertDialog.Builder(this)
                    .setTitle(item.title)
                    .setItems(options.toTypedArray()) { _, which ->
                        val selected = options[which]
